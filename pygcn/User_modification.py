...
class DataEntryForm(tk.Frame):
    def __init__(self, master,database = None,data = None):
        ...
        tk.Radiobutton(radio_frame, text='Non-binary', variable=gender_str_var, value=-1).pack(side=tk.LEFT, padx=5)
        ...
        information = Database.get_username(self.database,self.previous_data['username'])
        username_str_var.set(information['username'])
        email_str_var.set(information['email'])
        gender_str_var.set(information['gender'])
        interests = information.get('interests', [])  # 获取兴趣数组

        for hobby in hobby_list:
            if hobby[1] in interests:  # 如果兴趣字段存在于数组中
                hobby[0].set(1)  # 将对应的值置为 1
        ...
        def hit_return_button():
            ...
            self.destroy();
            AidedDiagnosis.AidedDiagnosis(master,data = previous_data)
        ...
        def hit_submit_button():
            ...
            if(submit_data()==True):
                submit_button.configure(state='normal')
                messagebox.showinfo(title='Successful Registration', message='User has been added successfully!')
        ...
        def submit_data():
            ...
            if(username_str_var.get()=="admin"):
                identity = "admin"
            data = {
                'original':self.previous_data['username'],
            ...
            if(Database.update_data(self.database,data)==True):
                self.previous_data['username'] = username_str_var.get()
                return True
            else:
                return False;
